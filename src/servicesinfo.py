...
class ServiceInfo (object):
    ...
    def __str__ (self):
        ...
        x = map (lambda x: '\t%s: %s' % (x, getattr (self, x)), filter (lambda y: hasattr (self, y), self.attrs))
...
class SysVServiceInfo (ServiceInfo):
    ...
    chkconfig_runlevels_re = re.compile (r'^\s*(?P<runlevels>\d+|-)\s+(?P<start>\d+)\s+(?P<stop>\d+)\s*$')
        ...
        for line in fd:
            if self.lsb_end_re.match (line):
                if lsb.startrunlevels.intersection (lsb.stoprunlevels):
                    ...
                    if len (lsb.stoprunlevels) == 0:
                        lsb.stoprunlevels = __allrunlevels__.difference (lsb.startrunlevels)
                    return lsb
            ...
            if m:
                ...
                if key == "Description":
                    ...
                    try:
                        lsb.startrunlevels = set (map (lambda x: int (x), value.split ()))
                    except ValueError:
                        return None
                ...
                if m:
                    ...
                    if key == "chkconfig":
                        ...
                        if m:
                            ...
                            chkconfig.stoprunlevels = __allrunlevels__.difference (chkconfig.startrunlevels)
